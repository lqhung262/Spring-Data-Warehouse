...
@Data
@SQLDelete(sql = "UPDATE attendance_machine SET is_deleted = true WHERE attendance_machine_id = ?")
@SQLRestriction("is_deleted = false")
public class AttendanceMachine {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "attendance_machine_id")
    private Long attendanceMachineId;

    @Column(name = "attendance_machine_code", length = 50, unique = true)
